...
product_details_twentytwo=[]
all_products_twentytwo=[]
...
olink="https://www.etb-tech.com/components/parts-upgrades/server-memory"
...
# if-else statment begins here
        ...
        if len(each)==24:
            ...
            product_details_twentytwo.append(product_info1)

            product_details_twentytwo.append(product_info2)
            product_details_twentytwo.append(each_product_link)

            for x in range(1,22,2):

                each_thing=each[x].text.strip()
                product_details_twentytwo.append(each_thing)
            all_products_twentytwo.append(product_details_twentytwo)
            product_details_twentytwo=[]
...
# Header for 24
header_twentyfour= ["Detail","Sub-Detail","Link Page", 'Manufacturer', 'Part Number', 'Capacity' ,"Memory Type", "Data Rate", "Peak Transfer Rate", "Rank", "Pins", "Voltage", "ECC", "Alternative Part Numbers" ,"Additional Information"]
...
header_twentytwo= ["Detail","Sub-Detail","Link Page", 'Chassis', 'Chassis Model', 'Chassis Type' ,"Power Supply", "Rack Rails", "Bezel", "Nodes", "CPU", "RAM", "Hard Drives", "Backplane","RAID Controller","Network Card","Remote Access","Motherboard Model", "Additional Information"]
with open( file_name+"twentytwo"+'.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    # write the header
    writer.writerow(header_twentytwo)
    # write multiple rows
    writer.writerows(all_products_twentytwo)
