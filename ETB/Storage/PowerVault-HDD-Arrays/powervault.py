...
product_details_sixteenOther=[]
all_products_sixteenOther=[]
...
counter1=0
olink="https://www.etb-tech.com/storage/powervault-hdd-arrays"
...
# There is a possibility of adding more if-else clauses to cover all different possibilities of descriptions
        ...
        if len(each)==16:
            ...
            if "Configure To Order" in product_info2:

                product_details_sixteenOther.append(product_info1)

                product_details_sixteenOther.append(product_info2)

                product_details_sixteenOther.append(each_product_link)

                for x in range(1,16,2):

                    each_thing=each[x].text.strip()
                    product_details_sixteenOther.append(each_thing)

                all_products_sixteenOther.append(product_details_sixteenOther)
                product_details_sixteenOther=[]
...
# Header for 28
header_fourteen= ["Detail","Sub-Detail","Link Page",'Hard Drives' ,"Backplane", "Controllers", "Power Supplies", "External RAID Controller", "Rail Kit" ,"Additional Information"]
...
with open( file_name+"fourteen"+'.csv', 'w', encoding='UTF8', newline='') as f:
    ...
    f.close()
header_sixteenOther= ["Detail","Sub-Detail","Link Page","Hard Drives", 'Backplane', 'Controllers', "Power Supplies", "External RAID Controllers", "External SAS Controllers", "Rail Kit","Additional Information"]
with open( file_name+"sixteenOther"+'.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    # write the header
    writer.writerow(header_sixteenOther)
    # write multiple rows
    writer.writerows(all_products_sixteenOther)
    f.close()
header_sixteen= ["Detail","Sub-Detail","Link Page","Hard Drives", 'Backplane', 'Controllers', "Power Supply",'Chassis Type', "Rack Rails", "Bezel","Additional Information"]
